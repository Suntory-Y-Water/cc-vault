...
export async function generateMetadata({
  ...
  const weeklyReport = generateWeeklyReport(selectedWeek);
  const title = `ウィークリーレポート - ${weeklyReport.weekRange.label}`;
  const description = `${weeklyReport.weekRange.label}の週間人気記事トップ10をご覧ください。`;
...
export default async function WeeklyReportPage({
  ...
  return (
    <div className='container mx-auto px-4 py-8'>
      ...
      {/* 週間人気記事トップ10 */}
      <TopArticles
        articles={weeklyReport.topArticles}
