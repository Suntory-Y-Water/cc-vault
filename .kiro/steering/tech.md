...
## アーキテクチャ概要
CC-VaultはCloudflare Workers上で動作する**マルチテナント・サーバーレス**Webアプリケーションです。Next.js App Routerを採用し、OpenNext.js Cloudflareによってエッジコンピューティング環境にデプロイされます。サブドメインベースのテナント識別により、各AIエージェント（Claude Code、Codex等）に専用の独立した情報体験を提供します。
...
## マルチテナント・サブドメインルーティング

### 🏗️ テナント識別・管理
- **ホスト名ベース解決**: Next.js `headers()` APIによるサブドメイン抽出
- **セキュアな検証**: ホスト名サニタイゼーション・バリデーション
- **AIエージェント設定**: `src/config/ai-agents.ts` による静的設定管理
- **フォールバック機能**: 未登録サブドメインのデフォルトテナント処理

### 🔐 テナント分離・セキュリティ
- **データベース分離**: Drizzle ORMクエリレベルでのテナントフィルタリング
- **クロステナント汚染防止**: 自動テナント識別子付与・検証
- **セキュリティ定数**: 最大サブドメイン長・正規表現パターン制御
- **権限チェック**: テナント境界の強制・アクセス制御

### 🎨 動的テーマ・UI管理
- **テナント別テーマ**: AIエージェント専用カラーパレット・デザイン
- **動的スタイリング**: Tailwind CSS クラスの条件分岐適用
- **ブランディング統合**: エージェント別ロゴ・UI要素カスタマイズ
- **レスポンシブ対応**: テナント別レスポンシブデザイン管理
...
### 🏢 マルチテナント設定
- **テナント設定**: `src/config/ai-agents.ts` による静的設定
- **ドメイン設定**: 本番・開発・ローカル環境別ドメイン管理
- **セキュリティ設定**: サブドメイン長・パターン制限の環境別調整
...
### 🏢 マルチテナント最適化
- **テナント別キャッシュ**: サブドメイン別コンテンツキャッシュ戦略
- **効率的なクエリ**: Drizzle ORMレベルでのテナントフィルタリング
- **設定キャッシュ**: AIエージェント設定のメモリキャッシュ
- **エッジ処理**: Cloudflare Workersでのテナント識別処理
